...
outPromoterMatrix="W137.heart.apex.s1-promoter_matrix.mtx.gz"

   python /net/trapnell/vol1/home/readdf/bin/bbi-sciatac-analyze/src/generate_sparse_matrix.py     --transposition_sites_intersect <(bedtools intersect -sorted -g W137.heart.apex.s1-human.chromosome_sizes.sorted.txt -a W137.heart.apex.s1-gene_regions.bed.gz -b W137.heart.apex.s1-transposition_sites.bed.gz -wa -wb)     --intervals W137.heart.apex.s1-gene_regions.bed.gz     --cell_whitelist W137.heart.apex.s1-called_cells_whitelist.txt     --matrix_output ${outPromoterMatrix}

   mv W137.heart.apex.s1-promoter_matrix.rows.txt promoter_matrix_rows.txt.no_metadata
   /net/trapnell/vol1/home/readdf/bin/bbi-sciatac-analyze/src/add_gene_metadata.py --in_promoter_matrix_row_name_file promoter_matrix_rows.txt.no_metadata                                        --gene_metadata_file /net/bbi/vol1/data/genomes_stage/human/human_atac/gene_bodies_gene_map.txt                                        --out_promoter_matrix_row_name_file W137.heart.apex.s1-promoter_matrix.rows.txt
