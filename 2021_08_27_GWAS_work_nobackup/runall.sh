...
# coaccessCut=0.015 # Cicero co-accessibility cutoff to count
# maxSites=20 # Max number of sites to link
# peakSize=600 # Size of distal sites
# upstream=0
# downstream=0
# exprMin=2
...
Rscript gatherInputFiles.R --promoterUpstream $upstream --promoterDownstream $downstream --coaccessCutoff $coaccessCut --maxNdistalSites $maxSites --peakSize $peakSize --cpmMin $exprMin --ratMin $logRatMin

# Sort 
# subPath="./fileOutputs/bedFilesForGWAS_"$upstream"_"$downstream"_"$coaccessCut"_"$maxSites"_"$peakSize"/"
# mkdir $subPath

bedfilePathBase="./fileOutputs/bedFilesForGWAS_"$upstream"_"$downstream"_"$coaccessCut"_"$maxSites"_"$peakSize"_"$exprMin"_"$logRatMin"/"$upstream"_"$downstream"_"$coaccessCut"_"$maxSites"_"$peakSize"_"$exprMin"_"$logRatMin"_"
samplesheetPath="./"$upstream"_"$downstream"_"$coaccessCut"_"$maxSites"_"$peakSize"_"$exprMin"_"$logRatMin"_samplesheet.txt"
...
do
	...
	bedtools merge -i $bedfilePathBase$cellFile"GeneAndPeakFileSorted.bed" > $bedfilePathBase$cellFile"GeneAndPeakFileSortedMerged.bed"
	bedtools sort -i $bedfilePathBase$cellFile"GeneAndPeakFileSortedMerged.bed" > $bedfilePathBase$cellFile"GeneAndPeakFileSorted.bed"
done
...
gwas_set="./ukbiobank_alkes_price_409k/ldsc_sumstats_files/relevantHeart_sumstat_metadata.txt"



output_directory="./fileOutputs/bedFilesForGWAS_"$upstream"_"$downstream"_"$coaccessCut"_"$maxSites"_"$peakSize"_"$exprMin"_"$logRatMin"/ldsc_results_full" #$$coaccessCut"_"$maxSites"_"$peakSize"/"$upstream"_"$downstream"_"$coaccessCut"_"$maxSites"_"$peakSize"_"
...
Rscript andrew_generate_figures.R --promoterUpstream $upstream --promoterDownstream $downstream --coaccessCutoff $coaccessCut --maxNdistalSites $maxSites --peakSize $peakSize --cpmMin $exprMin --ratMin $logRatMin

# # Clear old bed files
# tempBedDir="./fileOutputs/bedFilesForGWAS_"$upstream"_"$downstream"_"$coaccessCut"_"$maxSites"_"$peakSize"_"$exprMin"_"$logRatMin"_""/"
# rm $tempBedDir"/*.bed"
