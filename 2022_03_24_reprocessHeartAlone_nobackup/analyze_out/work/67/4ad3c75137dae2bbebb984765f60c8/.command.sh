...
outFragments="W134.heart.apex.s1-fragments.txt"
outTranspositionSites="W134.heart.apex.s1-transposition_sites.bed"
outInsertSizes="W134.heart.apex.s1-insert_sizes.txt"
outDuplicateReport="W134.heart.apex.s1-duplicate_report.txt"

python /net/trapnell/vol1/home/readdf/bin/bbi-sciatac-analyze/src/get_unique_fragments.py 		W134.heart.apex.s1-merged.bam 		--fragments ${outFragments} 		--transposition_sites_bed ${outTranspositionSites} 		--duplicate_read_counts ${outDuplicateReport} 		--insert_sizes ${outInsertSizes}
...
tabix -p bed ${outTranspositionSites}.gz
   ...
   outBedGraph="W134.heart.apex.s1-read_alignments.bedgraph"
   outBigWig="W134.heart.apex.s1-read_alignments.bw"

   bedtools genomecov -ibam W134.heart.apex.s1-merged.bam -bga | awk 'BEGIN{OFS="	"}{$1="chr" $1}1' > ${outBedGraph}
   # bedGraphToBigWig ${outBedGraph} W134.heart.apex.s1-human.chromosome_sizes.sorted.txt ${outBigWig}
