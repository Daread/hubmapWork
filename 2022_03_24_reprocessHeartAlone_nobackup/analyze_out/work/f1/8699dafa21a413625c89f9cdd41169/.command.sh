...
outFragments="W139.heart.RV.s1-fragments.txt"
outTranspositionSites="W139.heart.RV.s1-transposition_sites.bed"
outInsertSizes="W139.heart.RV.s1-insert_sizes.txt"
outDuplicateReport="W139.heart.RV.s1-duplicate_report.txt"

python /net/trapnell/vol1/home/readdf/bin/bbi-sciatac-analyze/src/get_unique_fragments.py 		W139.heart.RV.s1-merged.bam 		--fragments ${outFragments} 		--transposition_sites_bed ${outTranspositionSites} 		--duplicate_read_counts ${outDuplicateReport} 		--insert_sizes ${outInsertSizes}
...
tabix -p bed ${outTranspositionSites}.gz
   ...
   outBedGraph="W139.heart.RV.s1-read_alignments.bedgraph"
   outBigWig="W139.heart.RV.s1-read_alignments.bw"

   bedtools genomecov -ibam W139.heart.RV.s1-merged.bam -bga | awk 'BEGIN{OFS="	"}{$1="chr" $1}1' > ${outBedGraph}
   # bedGraphToBigWig ${outBedGraph} W139.heart.RV.s1-human.chromosome_sizes.sorted.txt ${outBigWig}
