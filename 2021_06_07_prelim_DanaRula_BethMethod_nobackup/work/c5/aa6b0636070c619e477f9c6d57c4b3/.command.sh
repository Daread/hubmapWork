...
cell_qc <- read.csv("Lung_cell_qc.csv")

if(nrow(pData(cds)) > 0) {
    if("false" == 'false') {
        scrublet_out <- read.csv("Lung_scrublet_out.csv", header=F)
...
write.csv(cell_qc, quote=FALSE, file="temp_fold/Lung_cell_qc.csv")

dup_stats <- read.table(paste0("Lung", ".duplication_rate_stats.txt"))

df <- data.frame(sample="Lung", n.reads = dup_stats$V2, n.umi = dup_stats$V3,
...
write.csv(df, file=paste0("Lung", "_sample_stats.csv"), quote=FALSE, row.names=FALSE)
saveRDS(cds, file="temp_fold/Lung_cds.RDS")
...
if ("Lung" == "Barnyard") {
    ...
    writeLines(paste0("Lung", "	", collision_rate, "%"), fileConn)
    close(fileConn)
} else {
    fileConn<-file("Lung_no_collision.txt")
    writeLines(paste0("Lung", "	", "NA"), fileConn)
    close(fileConn)
}
