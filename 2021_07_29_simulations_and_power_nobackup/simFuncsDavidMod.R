...
DiscreteEVF <- function(phyla, ncells_total, min_popsize, i_minpop, Sigma, n_nd_evf, n_de_evf, 
  ...
  evfs <- lapply(1:3, function(iparam){
    ...
    # DFR: 12-30-2020: Setting breakpoint here
    # browser()
...
# Start of functions that are written to allow for user-defined values of average expression.
# The code as written wants cell populations described as similar/different in fairly
#    opaque terms, using a phylogeny tree structure and branch lengths. This is 
#    going to be hard to interpret and difficult to meld w/ any other simulation
#    framework that uses simpler models like "draw from negative binomial with different means".
#    
# My approach will be to take a given change in distribution between two true populations,
#    simulate individuals according to some difference about these means,
#    then sample individaul cells for each individual with respect to these individual means.
#    Then I'll use the symSim infrastructure to generate true cell counts for these cells,
#    and finally generate observed counts based on expected UMIs, etc.
#
# Notably, the trickiest part of the initial "set up populations/individuals/cells" is 
#    that the symSim framework looks at transcription in terms of kon/koff/Synthesis (S) 
#    parameters for a two-state stochastic model. This helps generate data that 
#    fits the noise distribution intrinsic to bursty transcription, but it's a bit of a pain
#    to get parameter combos to give a more intuitive readout of four-fold rise

# Requires: foldChangeTwoOverOne is a number representing the fold change (not log FC)
#      of popTwoAverage/popOneAverage. 
#     empiricalDensities is the densities of kinetic params to use. Easiest is to use
#         getZeiselDensities() to give the densities from zeisel et al, as are used
#         as a standard in the original symSim publication.
#     popOneExprLevel is an optional paramter that sets the actual value of 
#       the popOne expression expected value, in terms of mean transcripts at 
#       steady state. If left blank, the function will just
#       draw parameters for kon/koff that both populations will use, then
#       vary "S" (synthesis rate) to give the fold-change needed. 
#     If popOneExprLevel is defined, it'll try to fit a different S for both
#       pops to both give the right fold change and set the popOne level
#     to the right level.
drawKineticParams <- function(foldChangeTwoOverOne, empiricalDensities, popOneExprLevel=NULL){

  # Sample.
  # When no constraint on popOne exact level, just draw kon/off, then S to give a difference
  if (is.null(popOneExprLevel)){
    kOn = SampleDen(nsample=1, den_fun=empiricalDensities[[1]])
    kOff = SampleDen(nsample=1, den_fun=empiricalDensities[[2]])
    # Now, given the on/off values we draw S values for the two populations.
    # Peak of the S distribution is around 2. Go with 
    #   2 +/- (log10(FC)/2)
    sOne = 2.0 - (log10(foldChangeTwoOverOne) / 2)
    sTwo = 2.0 + (log10(foldChangeTwoOverOne) / 2)
  }
  # TODO: Implement version that tries sampling to get a given popOneExprLevel, within
  #    the bounds of reasonable S parameters

  paramVec = c(kOn, kOff, sOne, kOn, kOff, sTwo)
  names(paramVec) = c("kOnOne", "kOffOne", "sOne", "kOnTwo", "kOffTwo", "sTwo")
  return(paramVec)
}

getZeiselDensities <- function(param_realdata="zeisel.imputed"){
  if(param_realdata=="zeisel.imputed"){
      data(param_realdata.zeisel.imputed)
    } else {stop("wrong input for parameter param_realdata")}
    match_params[,1]=log(base=10,match_params[,1])
    ...
    return(match_params_den)
}

# 
drawFromDist <- function(nToDraw, drawingDistribution){
  drawVec = rep(NA, nToDraw)

  for (eachInt in 1:nToDraw){
    drawVec[eachInt] = r(drawingDistribution)(1)
  }

  return(drawVec)
}

# Wrapper function to simulate draws from two populations
#   meanFoldChange: Fold change in mean expression between populations
#   kineticDensities: empirical distributions of kon/off/S, for sampling
#   indivNoiseDist: Distribution of S, in log10 scale, about pop mean
#   cellNoiseDist: Distirbution of S, in log10 scale, about individual mean
#   indivsPerPop: Individuals per group
#   cellsPerIndiv: Cells per unique individual
getCellParamsFromPops <- function(meanFoldChange, kineticDensities, indivNoiseDist,
                            cellNoiseDist, indivsPerPop, cellsPerIndiv,
                            popOneSetExpressionLevel = NULL, nGenes=1, randomSeed=7,
                            bimod=0){
  
  set.seed(randomSeed)
  # Here, I'm assuming that all noise at the individual and cell level is independent
  #    of other noise. Can consider changing this later
  totalCells = (2 * indivsPerPop * cellsPerIndiv)

  # Make the matrix to hold each parameter
  cellParams = list(array(numeric(),dim= c(totalCells,nGenes) ),
                    array(numeric(),dim= c(totalCells,nGenes)),
                    array(numeric(),dim= c(totalCells,nGenes)) )

  # Loop for each
  for (eachGeneInd in 1:nGenes){
    # Get the overall parameters
    populationParams = drawKineticParams(meanFoldChange, kineticDensities)  

    noiseVecIndiv = rep(NA, totalCells)
    noiseVecCells = rep(NA, totalCells)
    # Structure: All n cells for each m individuals in group one. Then all n cells per m individual
    #    in group 2.

    # Get individual noise
    noiseVecIndiv = rep(drawFromDist(2*indivsPerPop, indivNoiseDist), each=cellsPerIndiv)
    # Cells are all independent noise
    noiseVecCells = drawFromDist(totalCells, cellNoiseDist)
    # Adding noise from both sources to population means -> individual cell kinetic values
    populationBaseline = rep(c(populationParams["sOne"], populationParams["sTwo"]), 
                              each = (indivsPerPop*cellsPerIndiv))
    cellTrueSvals = populationBaseline + noiseVecIndiv + noiseVecCells

    # Added 2-3-2021
    # If any happen to be zero... fix that
    cellTrueSvals[cellTrueSvals < 0.2] <- .2

    # Now, setup the whole kinetic parameter lists
    cellsPerGroup = indivsPerPop*cellsPerIndiv
    kOnVec = c(rep(populationParams["kOnOne"], cellsPerGroup),
              rep(populationParams["kOnTwo"], cellsPerGroup))
    kOffVec = c(rep(populationParams["kOffOne"], cellsPerGroup),
              rep(populationParams["kOffTwo"], cellsPerGroup))

    # Fill the column of the cellParams array
    cellParams[[1]][,eachGeneInd] = kOnVec
    cellParams[[2]][,eachGeneInd] = kOffVec
    cellParams[[3]][,eachGeneInd] = cellTrueSvals
  }

  # Scale these
  cellParams <- Rescale_drawnParams(cellParams, bimod=bimod)
...
#' @import phytools
#' @export
getTrueCountsGivenParams  <- function(#ncells_total,min_popsize,
                inputParams, ncells_total=ncol(inputParams[[1]]),
                i_minpop=1,ngenes=1, 
                               evf_center=1,evf_type="one.population",nevf=10,
                               phyla, randseed, n_de_evf=0,vary='s',Sigma=0.4,
                               geffect_mean=0,gene_effects_sd=1,gene_effect_prob=0.3,
                               ...
                               gene_module_prop=0,prop_hge=0.015, mean_hge=5){
  set.seed(randseed)
  # params <- list(t(inputParams[[1]]), t(inputParams[[2]]), t(inputParams[[3]]) )
  params <- inputParams

  print("Got parameters")
  # David: Set up parameters here
  ...
  if (prop_hge>0){
    chosen_hge <- sample(ngenes, 
      # DFR: Mod
      max( c(2, ceiling(ngenes*prop_hge))),
           replace = F)
    multi_factors <- numeric(length(chosen_hge))
    rank_sum <- rank(rowSums(params[[3]][chosen_hge,]))
    multi_factors <- sapply(1:length(chosen_hge), function(igene){
  ...
  return(list(counts=counts, #gene_effects=gene_effects,cell_meta=cell_meta,
   kinetic_params=params #, in_module=is_in_module
   ))
}

# Function, adapting some code from Scale_Parameters in SymSim more or less verbatim,
#     to take kinetic values on log10 scale and convert them to linear scale, plus
#     (if desired) add shifts in kon/koff to 
#' This function takes gene_effect and EVF, take their dot product and scale the product to the correct range 
#' by using first a logistic function and then adding/dividing by constant to their correct range
#' @param gene_effects a list of three matrices (generated using the GeneEffects function), 
#' each corresponding to one kinetic parameter. Each matrix has nevf columns, and ngenes rows. 
...
#' @examples 
#' Get_params()
Rescale_drawnParams <- function(inputParams,
                            bimod=0,scale_s=1){
  # params <- lapply(1:3, function(iparam){evf[[iparam]] %*% t(gene_effects[[iparam]])})
  # scaled_params <- lapply(c(1:3),function(i){
  #   X <- params[[i]]
...
#' @import SummarizedExperiment
#' @export
GetObsCounts_Given_Param_In <- function(true_counts, #meta_cell,
                protocol="UMI",alpha_mean=0.1,alpha_sd=0.002,
                                gene_len,depth_mean, depth_sd, lenslope=0.02,nbins=20,
                                amp_bias_limit=c(-0.2, 0.2),
                                rate_2PCR=0.8,nPCR1=16, nPCR2=10, LinearAmp=F, LinearAmp_coef=2000){  
  # if(!is.null(SE)){
  ...
  if (protocol=="UMI"){return(list(counts=observed_counts#, cell_meta=meta_cell, 
    #nreads_perUMI=nreads_perUMI, 
                                  # nUMI2seq=nUMI2seq
                                   ) )
  } else
    return(list(counts=observed_counts, cell_meta=meta_cell))
...
processParamArgs <- function(){
  # Return a list that sets up some of the key parameters for runSimulations.R
  #     Set defaults such that non-specified params revert to baseline values

  # Set up a list of option objects:
  option_list = list(
  ...
  make_option(c("-v", "--indSD"), action="store", default=.1, type="numeric",
        help="standard deviation of individuals' true vals in log10 scale"),
  make_option(c("-n", "--ngenes"), action="store", default=50, type="integer",
        help="the number of genes to simulate"),
  make_option(c("-m", "--cellSD"), action="store", default=.02, type="numeric",
        help="the sd of cells' true vals in log10 scale")
  ) # end opttion_list
