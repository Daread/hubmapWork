...
def liftover(coordinates, liftover_path="/net/gs/vol1/home/ajh24/bin/ucsc_tools/liftOver", chain_file="/net/gs/vol1/home/ajh24/common_data/ucsc/goldenPath/hg19/liftOver/hg19ToMm9.over.chain.gz"):
	"""
	...
	for chrom, start, end, snp_id in coordinates:
		temp_file.write('\t'.join([chrom, str(start), str(end), snp_id]) + '\n')
	temp_file.close()
...
def pass_through_liftover_input(coordinates):
	"""
	Helper function to just format coordinates in same manner as liftover function when no liftover required.
	"""

	results = {}
	for chrom, start, end, snp_id in coordinates:
		 results[snp_id] = (chrom, start, end, snp_id)
	return results
...
if __name__ == '__main__':
	...
	parser.add_argument('--lift_over_path', default='/net/gs/vol1/home/ajh24/bin/ucsc_tools/liftOver', help='liftOver tool path.')
	parser.add_argument('--liftover_chain', help='Chain file for liftOver. If none specified, coordinates are passed through as is.')
	args = parser.parse_args()

	# Get RSID and corresponding positions (and genetic map position)
	rsids = parse_input_file(gzip.open(args.input_file, 'rt'))
	
	# Liftover all SNPs if requested
	liftover_input = []
	...
	if args.liftover_chain:
  		liftover_output = liftover(liftover_input)
	else:
		liftover_output = pass_through_liftover_input(liftover_input)
