...
outPeakMatrix="W137.heart.apex.s1-peak_matrix.mtx.gz"

   python /net/trapnell/vol1/home/readdf/bin/bbi-sciatac-analyze/src/generate_sparse_matrix.py     --transposition_sites_intersect <(bedtools intersect -sorted -g W137.heart.apex.s1-human.chromosome_sizes.sorted.txt -a W137.heart.apex.s1-merged_peaks.bed -b W137.heart.apex.s1-transposition_sites.bed.gz -wa -wb)     --intervals W137.heart.apex.s1-merged_peaks.bed     --cell_whitelist W137.heart.apex.s1-called_cells_whitelist.txt     --matrix_output ${outPeakMatrix}
