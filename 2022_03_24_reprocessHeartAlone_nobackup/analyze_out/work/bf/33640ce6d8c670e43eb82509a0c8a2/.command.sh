...
outPeakMatrix="W146.heart.apex.s1-peak_matrix.mtx.gz"

   python /net/trapnell/vol1/home/readdf/bin/bbi-sciatac-analyze/src/generate_sparse_matrix.py     --transposition_sites_intersect <(bedtools intersect -sorted -g W146.heart.apex.s1-human.chromosome_sizes.sorted.txt -a W146.heart.apex.s1-merged_peaks.bed -b W146.heart.apex.s1-transposition_sites.bed.gz -wa -wb)     --intervals W146.heart.apex.s1-merged_peaks.bed     --cell_whitelist W146.heart.apex.s1-called_cells_whitelist.txt     --matrix_output ${outPeakMatrix}
