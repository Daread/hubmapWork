...
oldCDS_Path = paste0("/net/trapnell/vol1/home/readdf/trapLabDir/hubmap/results/2022_08_30_revisitProportionsMixedModeling_nobackup/plots/", oldProcNote, "/")
...
getMixedBetaBinomialPropFits <- function(inputDF, modelFormula="Count ~ Anatomical_Site + Sex + Age", randomFormula = "~Donor"){
	# Loop and fit a model per cell type
	cellTypesToTest = as.character(unique(inputDF$Cell_Type))
	inputDF$Cell_Type = as.character(inputDF$Cell_Type)
	fitResList = vector(mode="list", length(cellTypesToTest))
