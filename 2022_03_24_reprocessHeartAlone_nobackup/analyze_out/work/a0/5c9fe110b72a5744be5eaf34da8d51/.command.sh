...
PROCESS_BLOCK='makeReducedDimensionMatrixProcess'
   ...
   inPeakMatrix="W146.heart.apex.s1-peak_matrix.mtx.gz"
   inSampleName="W146.heart.apex.s1"

	outScrubletHistFile="W146.heart.apex.s1-scrublet_hist.png"
   outScrubletTableFile="W146.heart.apex.s1-scrublet_table.csv"
outLsiCoordsFile="W146.heart.apex.s1-lsi_coords.txt"
outUmapCoordsFile="W146.heart.apex.s1-umap_coords.txt"
outUmapPlotFile="W146.heart.apex.s1-umap_plot"
outMonocle3CdsFile="W146.heart.apex.s1-monocle3_cds.rds"
   outBlackListRegionsFile="W146.heart.apex.s1-blacklist_regions_file.log"
   outReduceDimensionsLogFile="W146.heart.apex.s1-reduce_dimensions.log"
   umi_cutoff=100
   frip_cutoff=0.1
   frit_cutoff=0.05
...
Rscript /net/trapnell/vol1/home/readdf/bin/bbi-sciatac-analyze/src/reduce_dimensions.R     --sample_name ${inSampleName} 	--mat_file ${inPeakMatrix}     --count_file W146.heart.apex.s1-count_report.txt     --umi_cutoff ${umi_cutoff}     --frip_cutoff ${frip_cutoff}     --frit_cutoff ${frit_cutoff}     --doublet_predict_top_ntile ${doublet_predict_top_ntile}     --num_lsi_dimensions ${num_lsi_dimensions}     --cluster_resolution ${cluster_resolution}     --combined_read_count W146.heart.apex.s1-combined.duplicate_report.txt     --cds_file ${outMonocle3CdsFile}     --lsi_coords_file ${outLsiCoordsFile}     --umap_coords_file ${outUmapCoordsFile}     --umap_plot_file ${outUmapPlotFile} ${doublet_predict} ${black_list_file}
