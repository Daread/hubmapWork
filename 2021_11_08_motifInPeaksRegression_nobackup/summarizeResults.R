...
plotCellMarkers <- function(inputCSV, plotNote, markersToPlot, cellTypesToPlot, opt, onlyPosCoefs = TRUE){
	...
	myPlot = ggplot(inputCSV, aes_string(x="cellType", y="gene", size="coefficientValue")) + 
					geom_point() + 
...
markerSet = c("MEF2B", "SPIC", "CEBPA", "ERG", "ETV2", "EBF1", "TBX21")
plotCellMarkers(combinedCellTypeCSV, "canonicalMarkers", markerSet, cellTypes, opt)
