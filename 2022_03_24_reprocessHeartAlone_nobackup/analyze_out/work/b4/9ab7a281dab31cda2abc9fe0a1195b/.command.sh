...
outDir="call_peaks"
outMacsNarrowPeak="${outDir}/W136.heart.apex.s1_peaks.narrowPeak"
outNarrowPeak="W136.heart.apex.s1-peaks.narrowPeak.gz"
outMacsXls="${outDir}/W136.heart.apex.s1_peaks.xls"
outXls="W136.heart.apex.s1-peaks.xls"
outMacsSummits="${outDir}/W136.heart.apex.s1_summits.bed"
outSummits="W136.heart.apex.s1-summits.bed"

   # We used to add --shift -100 and --extsize, but the regions are now pre-shifted and extended
   # as output by other stages (ajh).
macs2 callpeak -t W136.heart.apex.s1-transposition_sites.bed.gz 		-f BED 		-g hs 		--nomodel 		--shift -100 		--extsize 200 		--keep-dup all 		--call-summits 		-n W136.heart.apex.s1 		--outdir ${outDir} 2> /dev/null
