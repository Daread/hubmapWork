...
outCounts="W145.heart.apex.s1-tss_counts.txt"
tmpRegions="W145.heart.apex.s1-temp_regions.gz"

   # TODO simplify this... maybe just have a stage that makes this file for TSS rather than complicating the stage itself

   bedtools slop -i W145.heart.apex.s1-human.tss_file.sorted.bed.gz -g W145.heart.apex.s1-human.chromosome_sizes.sorted.txt -b 1000     | bedtools merge -i stdin     | gzip > ${tmpRegions}

   python /net/trapnell/vol1/home/readdf/bin/bbi-sciatac-analyze/src/get_region_counts.py         --transposition_sites_intersect <(bedtools intersect -sorted -a W145.heart.apex.s1-transposition_sites.bed.gz -b ${tmpRegions})         --output_file ${outCounts}
