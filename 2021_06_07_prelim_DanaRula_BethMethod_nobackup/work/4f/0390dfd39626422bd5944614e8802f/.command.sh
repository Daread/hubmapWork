...
cds <- readRDS("Pancreas_cds.RDS")
cell_qc <- read.csv("Pancreas_cell_qc.csv")

if(nrow(pData(cds)) > 0) {
    if("false" == 'false') {
        scrublet_out <- read.csv("Pancreas_scrublet_out.csv", header=F)
...
write.csv(cell_qc, quote=FALSE, file="temp_fold/Pancreas_cell_qc.csv")

dup_stats <- read.table(paste0("Pancreas", ".duplication_rate_stats.txt"))

df <- data.frame(sample="Pancreas", n.reads = dup_stats$V2, n.umi = dup_stats$V3,
...
write.csv(df, file=paste0("Pancreas", "_sample_stats.csv"), quote=FALSE, row.names=FALSE)
saveRDS(cds, file="temp_fold/Pancreas_cds.RDS")

if ("Pancreas" == "Barnyard") {
    ...
    writeLines(paste0("Pancreas", "	", collision_rate, "%"), fileConn)
    close(fileConn)
} else {
    fileConn<-file("Pancreas_no_collision.txt")
    writeLines(paste0("Pancreas", "	", "NA"), fileConn)
    close(fileConn)
}
