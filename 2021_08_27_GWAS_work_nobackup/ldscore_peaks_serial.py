...
if __name__ == '__main__':
    ...
    for chromosome in range(1, 23):
        ...
        command = 'python /net/trapnell/vol1/ajh24/proj/2017mouse_cell_atlas/bin/ldscore_regression/generate_ld_score_annotation_template.py %s %s --peaks %s --buffer 100' % (original_template, template_file, args.master_peaks)
    ...
    #         # David modifying here to test outputs/errors
    #         errorDir = os.path.join(train_models_dir, ("outputFileDFR_" + sample['sample_id'] + "_" + str(chromosome)))
    #         # command = 'python /net/trapnell/vol1/ajh24/proj/2017mouse_cell_atlas/bin/ldscore_regression/generate_cluster_ldsc_model.py %s %s %s --bfile %s --hapmap_snps %s &> %s' % (template_file, sample['sites'], model_prefix, bfile, hapmap_file, errorDir)
            
    #         print("Running sample ")
    #         command = 'python /net/trapnell/vol1/home/readdf/trapLabDir/hubmap/results/2021_08_27_GWAS_work_nobackup/ldscore_regression_scripts/generate_cluster_ldsc_model.py %s %s %s --bfile %s --hapmap_snps %s &> %s' % (template_file, sample['sites'], model_prefix, bfile, hapmap_file, errorDir)
    #         print(command)
    ...
    #         # Andrew's original:
    #         # command = 'python /net/trapnell/vol1/ajh24/proj/2017mouse_cell_atlas/bin/ldscore_regression/generate_cluster_ldsc_model.py %s %s %s --bfile %s --hapmap_snps %s' % (template_file, sample['sites'], model_prefix, bfile, hapmap_file)
    ...
    # command = 'python /net/trapnell/vol1/ajh24/proj/2017mouse_cell_atlas/bin/ldscore_regression/gather_score_sumstats_results.py %s %s' % (results_file_list, results_file_gathered)
