...
out_dir = paste0(basepath, "archr/results/NB2_Peak_UMAP/")
dir.create(out_dir)
setwd(out_dir)
...
useMNN = TRUE
if (useMNN){
  processingNoteWithMNN = paste0(processingNote, "MNN_sample")
}

dropFirstComponent = FALSE
...
for(s in samples){
  ...
  plotUMAP_Monocle(thisCDS, paste0("Peak_CDS_", s, "_", processingNoteWithMNN), 
            "sampleName", outputPath=out_dir, show_labels=FALSE)
 }

shuffleCDS = thisCDS[,sample(ncol(thisCDS))]

plotUMAP_Monocle(shuffleCDS, paste0("Peak_CDS_", s, "_", processingNoteWithMNN), 
            "sampleName", outputPath=out_dir, show_labels=FALSE)
plotUMAP_Monocle(shuffleCDS, paste0("Peak_CDS_", s, "_", processingNoteWithMNN), 
            "FRIT", outputPath=out_dir, show_labels=FALSE)
plotUMAP_Monocle(shuffleCDS, paste0("Peak_CDS_", s, "_", processingNoteWithMNN), 
            "FRIP", outputPath=out_dir, show_labels=FALSE)
plotUMAP_Monocle(shuffleCDS, paste0("Peak_CDS_", s, "_", processingNoteWithMNN), 
            "log10umi", outputPath=out_dir, show_labels=FALSE)
plotUMAP_Monocle(shuffleCDS, paste0("Peak_CDS_", s, "_", processingNoteWithMNN), 
            "doublet_likelihood", outputPath=out_dir, show_labels=FALSE)



# Save this CDS, after dimensionality reduction
save(thisCDS, file=paste0(basepath, "cds_objects/cds_p_", s, "afterUMAP", processingNoteWithMNN))
