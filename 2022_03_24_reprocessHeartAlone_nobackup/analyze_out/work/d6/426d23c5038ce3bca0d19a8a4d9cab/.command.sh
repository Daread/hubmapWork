...
outCoverage="W139.heart.apex.s1-tss_region_coverage.txt.gz"
tmpOut="W139.heart.apex.s1-temp_file.gz"
   ...
   bedtools slop -i W139.heart.apex.s1-human.tss_file.sorted.bed.gz -g W139.heart.apex.s1-human.chromosome_sizes.sorted.txt -b 1000      | bedtools coverage -sorted -d -a stdin -b W139.heart.apex.s1-transposition_sites.bed.gz     | awk '{{ if ($8 > 0) print $0 }}'     | gzip > ${tmpOut}
