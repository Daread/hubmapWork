...
thisAlpha = .5
alphaToUse = c(.999) # Use a value close to 1 but not exactly, to avoid numerical issues that apparently can arise...? https://glmnet.stanford.edu/articles/glmnet.html
...
for (thisCellType in cellTypes){
  ...
  for (thisAlpha in alphaToUse){
    ...
    newRow = data.frame(thisCellType, thisAlpha, CVrSquared_bestLambda, trainRsquared)
    names(newRow) = c("Cell_Type", "Alpha", "CV_R_squared")
...
miniValY = valY[[paste0(thisCellType, opt$predictionTask)]]
...
accuracyOnVal = getAccuracy(trainCVfit, valX, miniValY)

# Get the coefficients from the best-fitting lambda
coef(trainCVfit, s= trainCVfit$lambda.min)

# Note: Seeing a strong SPIC signal in macrophages. Notable, distinct from it's described role in red pulp macrophages
