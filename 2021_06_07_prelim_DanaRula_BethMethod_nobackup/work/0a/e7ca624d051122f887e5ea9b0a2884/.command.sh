...
cds <- readRDS("Liver_cds.RDS")
cell_qc <- read.csv("Liver_cell_qc.csv")

if(nrow(pData(cds)) > 0) {
    if("false" == 'false') {
        scrublet_out <- read.csv("Liver_scrublet_out.csv", header=F)
...
write.csv(cell_qc, quote=FALSE, file="temp_fold/Liver_cell_qc.csv")

dup_stats <- read.table(paste0("Liver", ".duplication_rate_stats.txt"))

df <- data.frame(sample="Liver", n.reads = dup_stats$V2, n.umi = dup_stats$V3,
...
write.csv(df, file=paste0("Liver", "_sample_stats.csv"), quote=FALSE, row.names=FALSE)
saveRDS(cds, file="temp_fold/Liver_cds.RDS")
...
if ("Liver" == "Barnyard") {
    ...
    writeLines(paste0("Liver", "	", collision_rate, "%"), fileConn)
    close(fileConn)
} else {
    fileConn<-file("Liver_no_collision.txt")
    writeLines(paste0("Liver", "	", "NA"), fileConn)
    close(fileConn)
}
