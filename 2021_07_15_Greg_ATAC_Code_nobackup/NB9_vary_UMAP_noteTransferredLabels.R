...
# Add expression of levels of key genes for 
genesToPlotActivity = c("CD3D", "CD3E", "CD3G",  # T cell
            "NPR3", "SMOC1",                     # Endocardium
            "DCN",                               # Fibroblast
            "RGCC", "DLL4", "CA4",               # Vascular Endothelium
            "MYH11",                             # Vasc Smooth Musc
            "TTN", "FHL2",                       # Vardiomyocyte
            "KCNJ8" # Pericytes, look for more/better markers though
  )

for (eachGene in genesToPlotActivity){
  colData(cds.a.b)[eachGene] = log2(exprs(cds.a.g)[rowData(cds.a.g)$gene_short_name == eachGene,])
  colData(cds.a.g)[eachGene] = log2(exprs(cds.a.g)[rowData(cds.a.g)$gene_short_name == eachGene,])
}
