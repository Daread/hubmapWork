...
plotDir = "/net/trapnell/vol1/home/readdf/trapLabDir/hubmap/results/2021_07_15_Greg_ATAC_Code_nobackup/plots/QC_Plots/"
dir.create(plotDir)
...
for (row in 1:nrow(sample_df)){
  ...
  FRIP_co = sample_df[row, "FRIP_cutoff"]
  FRIT_co = sample_df[row, "FRIT_cutoff"]
  i = i + 1
...
# for (eachRow)
  ...
  qc_cells = filter(cDat_f, umi > 1000, FRIP > FRIP_co, FRIT > FRIT_co) %>% select(cell)
  ...
  save(cds_p, file = paste0(out_dir, "cds_p_", s))
}
